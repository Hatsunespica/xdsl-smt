...
extern "C" void bitWiseTemplateWrapped_c(APInt a, APInt b, APInt c, APInt d, int bnd, APInt result[2])
{
    ...
    for (int i = 0; i < 2; i++)
    {
        for (int j = 0; j < 2; j++)
        {
            ...
            // XOR logic: (~b_0 & ~a_0 & c_0 & d_0) | (a_0 & b_0 & ~d_0 & ~c_0)
            APInt notB_0 = ~b_0;
            APInt notA_0 = ~a_0;
            APInt notD_0 = ~d_0;
            APInt notC_0 = ~c_0;

            // First part: minAND(~b_0, ~a_0, c_0, d_0)
            APInt part1 = minAND(notB_0, notA_0, c_0, d_0);

            // Second part: minAND(a_0, b_0, ~d_0, ~c_0)
            APInt part2 = minAND(a_0, b_0, notD_0, notC_0);

            // Min result: maxOr(0, part1, 0, part2)
            APInt minResult = maxOr(zero, part1, zero, part2);

            // For max result
            // First part: maxAND(~b_0, ~a_0, c_0, d_0)
            APInt maxPart1 = maxAND(notB_0, notA_0, c_0, d_0);

            // Second part: maxAND(a_0, b_0, ~d_0, ~c_0)
            APInt maxPart2 = maxAND(a_0, b_0, notD_0, notC_0);

            // Max result: maxOr(0, maxPart1, 0, maxPart2)
            APInt maxResult = maxOr(zero, maxPart1, zero, maxPart2);
