...
//
//   std::set_intersection(kb_and_ucr.begin(), kb_and_ucr.end(),
//   scrVals.begin(),
...
public:
  ...
  const std::vector<Results> evalFinal(
    ...
    for (unsigned int i = 0; i < toEval.size(); ++i) {
      unsigned int bw = getBwa(toEval[i]);
      r.push_back({6, bw});
      ...
      for (unsigned int j = 0; j < toEval[i].size(); ++j) {
        ...
        // if (synthReduced.kb.isBottom() && !kbBest.isBottom() &&
        // !kbSynth.isBottom()) {}
