...
class UnsizedSolutionSet(SolutionSet):
    ...
    def learn_weights(self, context: SynthesizerContext):
        ...
        for i, sol in enumerate(self.solutions):
            ...
            self.logger.info(
                f"\tfunc {i}: #exact {res.exacts - res.unsolved_exacts} -> {res.exacts}, new exact%: {res.get_new_exact_prop()}, prec: {res.base_dist} -> {res.dist}, prec improve%: {res.get_improve()}, cond?: {self.solutions[i].cond is not None}, learn?: {to_learn}"
            )
            if to_learn:
