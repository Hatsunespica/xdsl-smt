...
for base_dir in base_dirs:
    for folder_name in sorted(os.listdir(base_dir)):
        ...
        if last_match:
            ...
            results.append(
                (folder_name, int(iter_num), float(exact), int(dis), base_dir)
            )
...
with open("table.tex", "w") as tex:
    ...
    print(
        r"  \textbf{concrete\_op} & \textbf{flags} & \multicolumn{2}{c}{\textbf{KnownBits}} & \multicolumn{2}{c}{\textbf{ConstantRange}} \\",
        file=tex,
    )
    print(
        r"                        &                & LLVM Exact\% & Our Exact\% & LLVM Exact\% & Our Exact\% \\",
        file=tex,
    )
    ...
    for idx, (disp, op_key, flags) in enumerate(rows):
        ...
        # print the row
        print(
            f"  {disp} & {flags} & {ll_kb_str} & {our_kb_str} & {ll_cr_str} & {our_cr_str} \\\\",
            file=tex,
        )
        if idx in mid_after:
            print(r"  \midrule", file=tex)
