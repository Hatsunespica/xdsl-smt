...
def plot(filepath: str, key_pattern: str, plot_udis: bool = True):
    ...
    b = int(b)
    row_pattern = re.compile(rf"{a}_(\d+)_{c}\t([\d.]+)%\t([\d.]+)%\t([\d.]+)\t([\d.]+)")
    ...
    ax1.axvline(x=b, color='gray', linestyle='--', linewidth=1.5, label=f'Best Step (b = {b})')
    ax1.text(b + 2, 95, f'b = {b}', rotation=90, verticalalignment='top', color='gray', fontsize=10)
    ...
    if plot_udis:
        ax2 = ax1.twinx()
        ax2.plot(indices, udis_vals, label='UDis (right axis)', color='tab:green',
